...
with tf.Graph().as_default():
    ...
    embeddings = [np.random.uniform(-1, 1, (40000, 1024)),np.random.uniform(-1, 1, (40000, 1024)) ]
    
    with tf.Session() as sess:
        sess.run(init_op)
        for p, embedding, assign_op in zip(placeholders, embeddings, assign_ops):
#            embedding = np. random.uniform(-1, 1, (40000, 1024))
            result = sess.run(assign_op, {p:embedding})
#            print(result)
            save_path = saver.save(sess, './v3/model.ckpt')
